...
{
	//상수 선언//
	private final int IMAGE = 16;//조각의 수 
	private final int XSIZE = 150;//각 조각의 X축 크기 
	private final int YSIZE = 112;//각 조각의 Y축 크기 
	/////////////
	...
	public void Init()//초기화 함수 
	{
		...
		while(check<1)//시작시 그림 랜덤 배치하도록 
		{
			...
			{
				image[i] = new ImageIcon("image_"+(i+1)+".jpg");//이미지 불러오기 
				imagelbl[j] = new JLabel("",image[i],SwingConstants.CENTER);//이미지를 라벨에 
				saveId[j] = i;//난수와 j변수를 통해 이미지가 들어가야 할 위치를 알아둠 
				imagelbl[j].setBounds(630,70 + YSIZE * (count/4),XSIZE,YSIZE);//이미지 위치 설정 
				startPt[j].x = 630;//시작 위치 설정 X 
				startPt[j].y = 70 + YSIZE * (count/4);//시작 위치 설정 Y
				ximageRange[j] = 630 + XSIZE;//끝 위치 설정 X 
				yimageRange[j] = 70 + YSIZE * (count/4) + YSIZE;//끝 위치 설정 Y
				add(imagelbl[j]);
	...
	public void InitframePt()//framePt변수를 생성,초기화하고 설정해줌 
	{
		...
		{	
			framePt[i] = new Point();
			framePt[i].x = 10 + XSIZE * (i%4);
			framePt[i].y = 70 + YSIZE * (int)(i/4);
			if(i>7)
			{
				framePt[i].y = 70 + YSIZE * (int)(i/4) - 2;
			}
		}
	}
